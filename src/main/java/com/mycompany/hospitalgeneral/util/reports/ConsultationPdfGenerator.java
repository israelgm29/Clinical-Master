...
/**
 * Generador de PDF para reportes de consulta médica Usa iText 9.5
 */
@Dependent
public class ConsultationPdfGenerator {
    ...
    private void buildHeader(Document doc, String reportNumber) {
        String companyName = company != null ? company.getName().toUpperCase() : "INSTITUCIÓN MÉDICA";
        String companyAddress = company != null && company.getAddress() != null ? company.getAddress() : "";
        String companyPhone = company != null
                ? (company.getTelephone() != null ? company.getTelephone()
                : (company.getMobile() != null ? company.getMobile() : "")) : "";
        String companyRuc = company != null && company.getRuc() != null ? "RUC: " + company.getRuc() : "";
        ...
        if (!companyAddress.isEmpty()) {
            contactParts.add(companyAddress);
        }
        if (!companyPhone.isEmpty()) {
            contactParts.add("Tel: " + companyPhone);
        }
        if (!companyRuc.isEmpty()) {
            contactParts.add(companyRuc);
        }
        if (!companyEmail.isEmpty()) {
            contactParts.add(companyEmail);
        }
        ...
        doc.add(new Paragraph("REPORTE DE CONSULTA MÉDICA")
                .setFont(regularFont)
        ...
        infoTable.addCell(new Cell()
                .add(new Paragraph("N° Reporte: " + reportNumber)
                        .setFont(boldFont).setFontSize(ReportStyles.FONT_SIZE_SMALL))
    ...
    private void buildPatientInfo(Document doc, Patient patient) {
        ...
        String[][] data = {
            ...
            {"HC:", patient.getHc()},
            {"Identificación:", patient.getDni() != null ? "CI: " + patient.getDni()
                : (patient.getPassport() != null ? "Pasaporte: " + patient.getPassport() : "N/A")},
            {"Fecha Nacimiento:", PdfGeneratorUtil.formatDate(patient.getBirthday())},
            {"Edad:", PdfGeneratorUtil.calculateAge(patient.getBirthday()) + " años"},
            {"Sexo:", patient.getSex() != null ? patient.getSex().getName() : "N/A"},
            {"Estado Civil:", patient.getCivilstatus() != null ? patient.getCivilstatus().getName() : "N/A"},
            {"Tipo de Sangre:", patient.getBlootype() != null ? patient.getBlootype().getName() : "N/A"},
            {"Dirección:", patient.getAddress() != null ? patient.getAddress() : "N/A"},
            {"Teléfono:", patient.getMobile() != null ? patient.getMobile()
    ...
    private void buildCros(Document doc, Medicalrecord record) {
        if (record.getCrosCollection() == null || record.getCrosCollection().isEmpty()) {
            return;
        }
    ...
    private void buildRpe(Document doc, Medicalrecord record) {
        if (record.getRpeCollection() == null || record.getRpeCollection().isEmpty()) {
            return;
        }
    ...
    private void buildSignatures(Document doc, Medicalrecord record) {
        ...
        // Firma médico
        Medic medic = record.getMedicid();
        String medicName = medic != null
                ? "Dr./Dra. " + medic.getLastname() + " " + medic.getFirstname()
                : "Médico no registrado";
        String regProfessional = medic != null && medic.getRegprofessional() != null
                ? "Reg. Prof.: " + medic.getRegprofessional() : "";
        String specialties = medic != null ? medic.getSpecialtiesNames() : "";

        Cell medicCell = new Cell()
                .add(new Paragraph("\n\n\n"))
                .add(new Paragraph("_________________________")
                        .setTextAlignment(TextAlignment.CENTER))
                .add(new Paragraph(medicName)
                        .setFont(boldFont).setFontSize(ReportStyles.FONT_SIZE_SMALL)
                        .setTextAlignment(TextAlignment.CENTER))
                .add(new Paragraph(specialties)
                        .setFont(regularFont).setFontSize(ReportStyles.FONT_SIZE_TINY)
                        .setFontColor(ReportStyles.TEXT_MUTED).setTextAlignment(TextAlignment.CENTER))
                .add(new Paragraph(regProfessional)
                        .setFont(regularFont).setFontSize(ReportStyles.FONT_SIZE_TINY)
                        .setFontColor(ReportStyles.TEXT_MUTED).setTextAlignment(TextAlignment.CENTER))
                .setBorder(Border.NO_BORDER);

        // Firma paciente
        String patientName = record.getPatientid() != null
                ? record.getPatientid().getLastname() + " " + record.getPatientid().getFirstname() : "Paciente";
        String patientId = record.getPatientid() != null && record.getPatientid().getDni() != null
                ? "CI: " + record.getPatientid().getDni() : "";
        ...
        Cell patientCell = new Cell()
                ...
                .add(new Paragraph(patientName)
                        .setFont(boldFont).setFontSize(ReportStyles.FONT_SIZE_SMALL)
                        .setTextAlignment(TextAlignment.CENTER))
                .add(new Paragraph(patientId)
                        .setFont(regularFont).setFontSize(ReportStyles.FONT_SIZE_TINY)
                        .setFontColor(ReportStyles.TEXT_MUTED).setTextAlignment(TextAlignment.CENTER))
                .add(new Paragraph("Firma del Paciente")
                        .setFont(regularFont).setFontSize(ReportStyles.FONT_SIZE_TINY)
                        .setTextAlignment(TextAlignment.CENTER))
                .setBorder(Border.NO_BORDER);
